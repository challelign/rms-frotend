...
const LegalPerson = ({}) => {
    ...
    },
    {
      ...
      cell: row =>
        row.id != null ? (
          <Button
            onClick={() =>
              editClicked(row.id, row.full_name, row.account_number, row.phone, row.credit_amount)
            }
          >
  ...
  const editFcyCustomer= () => {
    ...
    if (values.phone.trim() === '') {
      ...
      setErrorMessage2('Phone number length must be 10 digit. Example 0911111111');
    }
    else if (values.account_number.trim() !== '' && values.account_number.length != 16) {
