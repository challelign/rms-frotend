...
const Below500List = ({ values }) => {
...

    {
      ...
      cell: row =>
        row.id != null ? (
          <Button
            color="primary"
            onClick={() => click(row.id, row.customer_name, row.response, row.reason, row.remark, row.customer_address,row.other_reason)}
          >
            Reason
          </Button>
        ) : (
  ...
  const getBelow500 = () => {
    ...
    axios
      .get(`${url}/below_500/all/${page}/${countPerPage}`, {
